...
if (
  !empty(@$arrSubMenu) and 
  !preg_match('/index.php\?tpf=shop/',$_SERVER['REQUEST_URI']) and
  !preg_match('/\/member/',$_SERVER['REQUEST_URI'])
) {
?>
...
if (!empty($arrNav[substr(@$category_code,0,2)]['sub'])) { // 2depth 가 있을경우 표출
    echo '
    <div class="cateBox cateBox--2depth ce_item">
